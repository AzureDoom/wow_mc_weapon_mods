...
public class LootHandler {
	...
	private LootPool getInjectPool(String entryName) {
		return new LootPool(new LootEntry[] { getInjectEntry(entryName, 1) }, new LootCondition[0],
				new RandomValueRange(1), new RandomValueRange(0, 1), "wowweapons_inject_pool");
	}

	private LootEntryTable getInjectEntry(String name, int weight) {
		return new LootEntryTable(new ResourceLocation(WoWWeaponsMod.modid, "inject/" + name), weight, 0,
				new LootCondition[0], "wowweapons_inject_entry");
	}
